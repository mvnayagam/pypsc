...
from psc.lib.solutionIO import readh5file_v2

def plottotaltime(reflections: list, fpath: str, figname: str='ex', figtype: str='pdf', savefig: bool=True):
    ...
    reflections = reflections if type(reflections) == list else np.arange(2, reflections)
    
    for ci, ls in enumerate(reflections):
        fn = os.path.join(fpath,'pnew_%g.h5'%(ls))
        print(f'---> Now reading: {fn}')
        dataread=readh5file_v2(fn) 
        #return [0]np.array(volume), [1]mean[0], [2]error, [3]mean, [4]np.array(koordinate), [5]centroid, [6]radius, [7]noofsolution, [8]tinfo, [9]maxerror
        mmean.append(dataread[1])
        meanl.append(dataread[1])
        nsol.append(dataread[7])
        vave.append(dataread[1])
        emax.append(dataread[9][0])
        t.append(dataread[8][0][2:])
...
#         print(f'---> Now reading: {fn}')
#         dataread=readh5file_v2(fn)
#         mmean.append(dataread[1]) # return np.array(volume), mean[0], error, mean, np.array(koordinate), centroid, radius, noofsolution
#         meanl.append(dataread[1])
...
#             tti  = np.max(tt) #[np.max(np.array(tinf.get(ii)))for ii in np.array(tinf)]
#             t.append(tt[2:]); tave.append(tti)
...
#             nss = np.shape(np.array([f.get('allsolution/Pair'+str(ii)) for ii in range(0, len(ns)) ][0]))[0]
#             nsol.append(nss)
